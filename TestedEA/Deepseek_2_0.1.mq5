...
#include  <trade/trade.mqh>
   ...
   // Define parameters
   double lotSize = 0.1; // Risk management
   int atrPeriod = 14; // ATR period for dynamic stop-loss/take-profit
   double riskRewardRatio = 4; // Risk-reward ratio
   ENUM_TIMEFRAMES higherTFPeriod = PERIOD_M20; // Higher timeframe for trend confirmation
   int higherTFMAPeriod = 100; // Higher timeframe SMA period
   int Magic = 111222333;
...
void OnTick()
  {
   ...
   if (volumeDelta > 0 && iClose(_Symbol,higherTFPeriod,0) > higherTFMA && isBullishSentiment && !hasOpenPosition())
     {
      double stopLoss = iClose(_Symbol,higherTFPeriod,0) - atr*2;
      double takeProfit = iClose(_Symbol,higherTFPeriod,0) + (riskRewardRatio * atr * 2);
